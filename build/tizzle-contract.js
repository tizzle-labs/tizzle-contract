...
let TokenPurchase = (_dec = NearBindgen({}), _dec2 = initialize(), _dec3 = view(), _dec4 = view(), _dec5 = call({
  payableFunction: true
}), _dec6 = call({}), _dec(_class = (_class2 = class TokenPurchase {
  constructor() {
    this.owner = "";
  ...
  init({
    ...
    assert(owner.length > 0, "Pemilik harus ditentukan");
    this.owner = owner;
    this.contractAddress = currentAccountId();
  }
  getOwner() {
    return this.owner;
  }
  ...
  buyToken() {
    ...
    const depositAmount = attachedDeposit();
    assert(depositAmount > BigInt(0), "Jumlah transfer harus lebih besar dari 0");
    const initialBalance = accountBalance();
    log(`Saldo awal kontrak: ${initialBalance}`);
    log(`Deposit amount: ${depositAmount}`);
    log(`Menerima ${depositAmount} yoctoNEAR dari ${buyerAccountId} untuk pembelian token`);

    // Tidak perlu mentransfer NEAR secara eksplisit, karena sudah ditambahkan ke saldo kontrak
    // oleh runtime NEAR ketika fungsi payable dipanggil

    const finalBalance = accountBalance();
    log(`Saldo akhir kontrak: ${finalBalance}`);
    log(`Selisih saldo: ${BigInt(finalBalance) - BigInt(initialBalance)}`);
    log(`Token dibeli oleh ${buyerAccountId}. Jumlah NEAR: ${depositAmount}`);
  }
  setOwner({
    newOwner
  }) {
    assert(predecessorAccountId() === this.owner, "Hanya pemilik kontrak yang dapat mengubah pemilik");
    assert(newOwner.length > 0, "ID pemilik baru harus ditentukan");
    this.owner = newOwner;
    log(`Pemilik kontrak diubah menjadi ${newOwner}`);
  }
}, _applyDecoratedDescriptor(_class2.prototype, "init", [_dec2], Object.getOwnPropertyDescriptor(_class2.prototype, "init"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "getOwner", [_dec3], Object.getOwnPropertyDescriptor(_class2.prototype, "getOwner"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "balance", [_dec4], Object.getOwnPropertyDescriptor(_class2.prototype, "balance"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "buyToken", [_dec5], Object.getOwnPropertyDescriptor(_class2.prototype, "buyToken"), _class2.prototype), _applyDecoratedDescriptor(_class2.prototype, "setOwner", [_dec6], Object.getOwnPropertyDescriptor(_class2.prototype, "setOwner"), _class2.prototype), _class2)) || _class);
function setOwner() {
  const _state = TokenPurchase._getState();
  if (!_state && TokenPurchase._requireInit()) {
    throw new Error("Contract must be initialized");
  }
  const _contract = TokenPurchase._create();
  if (_state) {
    TokenPurchase._reconstruct(_contract, _state);
  }
  const _args = TokenPurchase._getArgs();
  const _result = _contract.setOwner(_args);
  TokenPurchase._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(TokenPurchase._serialize(_result, true));
}
function buyToken() {
  const _state = TokenPurchase._getState();
  if (!_state && TokenPurchase._requireInit()) {
  ...
  const _args = TokenPurchase._getArgs();
  const _result = _contract.buyToken(_args);
  TokenPurchase._saveToStorage(_contract);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(TokenPurchase._serialize(_result, true));
}
function balance() {
  ...
  const _args = TokenPurchase._getArgs();
  const _result = _contract.balance(_args);
  if (_result !== undefined) if (_result && _result.constructor && _result.constructor.name === "NearPromise") _result.onReturn();else env.value_return(TokenPurchase._serialize(_result, true));
}
function getOwner() {
  const _state = TokenPurchase._getState();
...
export { balance, buyToken, getOwner, init, setOwner };
//# sourceMappingURL=tizzle-contract.js.map
